...
function MyPage() {
    ...
    const fetchUser = async (email) => {
        try {
            ...
            const response = await fetch(`http://3.38.100.100:8080/api/user/mypage?email=${email}`, {
                method: 'GET',
                headers: {
    ...
    const fetchPurchaseHistory = async (email) => {
        try {
            setPurchaseHistory([]);
            const response = await fetch(`http://3.38.100.100:8080/api/orders?email=${email}`);
            if (!response.ok) {
    ...
    const handleConfirmCancel = async (orderId) => {
        try {
            toast.dismiss();
            const response = await fetch(`http://3.38.100.100:8080/api/orders/${orderId}`, { method: 'PATCH' });
            if (!response.ok) {
    ...
    const fetchUserGrade = async (email) => {
        try {
            const response = await fetch(`http://3.38.100.100:8080/api/user/${email}`);
            if (!response.ok) {
    ...
    const handleEdit = async (field) => {
        ...
        switch (field) {
            case 'address':
                ...
                endpoint = `http://3.38.100.100:8080/api/user/address/${email}`;
                break;
            case 'password':
                updatedValue = prompt('새 비밀번호를 입력해주세요:');
                endpoint = `http://3.38.100.100:8080/api/user/password/${email}`;
                break;
            default:
                return;
        }
