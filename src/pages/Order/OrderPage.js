...
function OrderPage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const response = await fetch(`http://3.38.100.100:8080/api/user/${userEmail}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://3.38.100.100:8080/api/orders?email=${userEmail}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // 장바구니 비우기 API 호출
        await fetch(`http://3.38.100.100:8080/api/cart?email=${userEmail}`, {
          method: 'DELETE',
        });
        ...
        // 쿠폰 비활성화 API 호출
        if (couponId) {
          await fetch(`http://3.38.100.100:8080/api/coupon/use/${couponId}`, {
            method: 'POST',
          });
